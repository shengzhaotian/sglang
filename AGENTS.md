...
1. Model Adaptation (sglang-model-adapter)
   ...
   └── Validate with dummy + real weights
              ↓
2. Comprehensive Testing (npu-testing-workflow)
   ├── Service deployment scripts
   ├── Client benchmarking
   └── Test report generation
```

**For detailed NPU configuration (environment variables, server arguments, features, troubleshooting)**, see the skill documentation.
