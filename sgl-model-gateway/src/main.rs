...
struct CliArgs {
    ...
    /// API key for control plane access with role (format: "role:key")
    /// Can be specified multiple times.
    /// Example: --control-plane-api-key "admin:secret123"
    #[arg(long, action = ArgAction::Append, env = "CONTROL_PLANE_API_KEY")]
    control_plane_api_key: Vec<String>,
...
/// Parse control plane API key from CLI format "role:key"
fn parse_control_plane_api_key(key_str: &str, index: usize) -> Option<ApiKeyEntry> {
    let parts: Vec<&str> = key_str.splitn(2, ':').collect();
    if parts.len() != 2 {
        eprintln!(
            "WARNING: Invalid control-plane-api-key format '{}'. Expected 'role:key'",
            key_str
        );
        return None;
    }
    ...
    Some(ApiKeyEntry::new(
        format!("cli-key-{}", index),
        format!("CLI API Key {}", index),
        key,
        role,
    ))
}
...
impl CliArgs {
    ...
    fn build_control_plane_auth_config(&self) -> ControlPlaneAuthConfig {
        ...
        let api_keys: Vec<ApiKeyEntry> = self
            ...
            .iter()
            .enumerate()
            .filter_map(|(i, k)| parse_control_plane_api_key(k, i))
            .collect();
