...
def _pool(*, dtype=torch.float8_e4m3fn, nz=False, flash=True, **kwargs):
    path = (
    ...
    namespace = {
        ...
        "get_bool_env_var": lambda name: (
            flash if name == "SGLANG_NPU_USE_FLASH_MLA" else nz
        ),
...
def test_split_allocation_dtype_size_shape_and_layer_offset(flash):
    pool = _pool(flash=flash)
...
@pytest.mark.parametrize("flash", [False, True])
def test_write_quantizes_only_latent_with_per_layer_scale(nz, flash):
    pool = _pool(nz=nz, flash=flash)
...
def test_bf16_preserves_upstream_non_flash_split_buffer_contract():
    pool = _pool(dtype=torch.bfloat16, flash=False)
    assert not pool.mla_kv_cache_store_fp8
    assert not hasattr(pool, "kv_buffer")
    latent, rope = _inputs(2)
    pool.set_kv_buffer(SimpleNamespace(layer_id=3), torch.tensor([4, 5]), latent, rope)
    key, side = pool.get_kv_buffer(3)
    assert key.dtype == side.dtype == torch.bfloat16
    assert key.shape == (5, 4, 1, 512)
    assert side.shape == (5, 4, 1, 64)
    assert pool.get_kv_size_bytes() == 2 * 20 * (512 + 64) * 2
    torch.testing.assert_close(key.reshape(-1, 1, 512)[4:6], latent)
    torch.testing.assert_close(side.reshape(-1, 1, 64)[4:6], rope)
