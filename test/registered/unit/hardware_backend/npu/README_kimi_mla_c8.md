# Kimi-K3 MLA C8: first A5 integration
...
- Shared latent K/V: FP8 E4M3, checkpoint per-tensor FP32 descale per layer.
  The checkpoint K and V scales must match; offsets must be zero.
...
nonfinite, nonpositive or inconsistent scales fail instead of defaulting to 1.
...
New target C8 graph, CP/A2A, ragged verify, disaggregation and CPU cache offload
are not supported by this first integration. Existing BF16 paths are unchanged.
...
Run the four `test_kimi_mla_fp8_*.py` files with pytest in an installed SGLang
...
prefill cache compatibility and decode/verify dispatch. They do not establish
that the installed CANN supports these operator shapes or that end-to-end A5
accuracy/performance meets the target.
